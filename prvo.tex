...
Para poder definir el operador \emph{PRVO}, primero necesitamos definir qu\'e consideramos una \emph{expresi\'on de navegaci\'on}. En t\'erminos generales, considerando a un elemento (variable, m\'etodo, campo, o literal) como una funci\'on $O \rightarrow D$, y dado un contexto $\rho$ que representa el conjunto de todos los elementos disponibles, una expresi\'on es un elemento o una secuencia de elementos $[e_1, e_2,..., e_n]$ mediante las siguientes reglas de inferencia:


\begin{definition}
	\infer{\rho, Expr [e] : O \rightarrow T}{
		\rho, \vdash e : O \rightarrow T
	}
	\infer{\rho, Expr [A]\:++\:Expr [B] : O \rightarrow T_2}{
		{\rho, \vdash Expr [A] : O \rightarrow T_1}
		& {\rho, \vdash Expr [B] : T_1 \rightarrow T_2}
	}
\end{definition}

Esta definici\'on garantiza el correcto tipado de una expresi\'on, y cuando una expresi\'on se forma por una secuencia de al menos dos elementos, \'esta se considera de navegaci\'on. En lenguajes orientados a objetos, el acceso a miembros de una clase es de uso com\'un, y se realiza mediante un operador de navegaci\'on que puede tomar distintas formas sint\'acticas, con la notaci\'on \emph{punto} siendo la m\'as com\'un. Esta notaci\'on permite encadenar expresiones, el tama\~no de una expresi\'on de navegaci\'on se puede definir de distintas formas; nosotros la definiremos por el n\'umero de operadores de navegaci\'on involucrados. Una expresi\'on de navegaci\'on de tama\~no 1 entonces tendr\'ia la forma \emph{a.b}. Ejemplos de expresiones de distintos tama\~nos con respecto al operador de navegaci\'on se pueden ver en la Figura \ref{figures.examples.chainedExpr}, donde vale aclarar que si una expresi\'on no contiene al menos un uso del operador de accesibildad a miembros, \'esta no se considera expresi\'on de navegaci\'on. Esta distinci\'on es importante cuando m\'as adelante se describa el tipo de expresiones que puede mutar \emph{PRVO}.
...
\begin{figure}
	\begin{displaymath}
	\begin{array}{lll}
	PRVO(Expr [e] : O \rightarrow T, \rho) & = & \{Expr [f] : O \rightarrow T \:|\: [f] \in (\rho)^\ast\} 
	\end{array}
	\end{displaymath}
