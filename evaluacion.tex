...
Los resultados obtenidos de \emph{Mutation Score} \ref{mutationscore-results}, \emph{Toughness} \ref{toughness-results}, y \emph{Toughness de solo detectados} \ref{toughnessKilled-results}, se condicen con los resultados de dominancia de \emph{prvo} y mutantes equivalentes generados. El mutation score al utilizar \emph{prvo} (en color rojo) se mantiene en general, con respecto al valor obtenido sin utilizar \emph{prvo} (en color verde), esto junto a los resultados de dominancia
indica que la cantidad de mutantes triviales generados por \emph{prvo} es poca (lo que llevar\'ia a un aumento de esta m\'etrica), y que la generaci\'on de mutantes equivalentes tambi\'en lo es, dado que \'estos llevar\'ian a una disminuci\'on del valor de mutation score obtenido.

Los resultados de \emph{Toughness}, tanto si se consideran todos los mutantes o solo se consideran aquellos que fueron detectados, apoyan la conclusi\'on anterior de que \emph{prvo} no solo que no genera mutantes triviales, sino que adem\'as, \'estos son en general dif\'iciles de matar. Para aquellos mutantes generados por \emph{prvo} que no fueron detectados y tampoco fueron considerados como equivalentes. si bien los experimentos realizados no alcanzan para afirmar que sean stubborn, la gran cantidad de tests con mayormente una cobertura de ramas alta \ref{testsAndCov-results} podr\'ia llevar a considerar a las test suites utilizadas como ``suficientemente buenas'', y en ese caso considerar a los mutantes no equivalentes y no detectados, como mutantes stubborn. Este resultado son coherentes con los obtenidos del an\'alisis de Dynamic Mutant Subsumption.
